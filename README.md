...
1. 准备初一、初二、初三、四高一和高一的 5 份 `.xlsx`、`.xls` 或 `.csv` 名单。
2. 打开网页，一次选择或拖入全部 5 份文件。程序根据文件名自动分为四组，并将初三与四高一合并。
3. 点击“开始抽奖”，每组每轮各抽取 3 人，共 12 人。默认会在下一轮排除已中奖观众。
4. 点击“复制结果图片”，可以直接粘贴到 WPS/PPT；不支持图片剪贴板的浏览器会自动下载 PNG。

程序会自动选择最符合“姓名＋学号”结构的工作表；如果没有表头，则将第一列当作姓名、第二列当作学号。重复学号和空白姓名会被自动忽略。
